...
export async function prepareRootCapability<Extra extends object = Record<never, never>>(
  ...
  const claims: BaseLeaseClaims & Extra = {
    lease_id: leaseId, subject_pubkey: policy.subject.publicKey, aud: policy.audience,
    ...
    resource: policy.resource, root_lease_id: leaseId, ...(extra ?? {} as Extra),
  }
  ...
  const result = await client.query(
    `INSERT INTO tool_leases
       (lease_id, subject_pubkey, hedera_tx_id, expires_at, max_calls, policy_hash,
        ...
        root_lease_id)
     VALUES ($1, $2, $3, to_timestamp($4), $5, $6, $7, $8, $9, $10, 2, $11, $12, $13)
     RETURNING lease_id`,
